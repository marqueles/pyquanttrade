...
class Trade:
    ...
    def close(self, day, price):
        ...
        if self.type == "long":
            ...
            self.profit = (
                self.num_actions * (self.close_price - self.open_price)
                - self.slippage
                - self.commission
            )
        else:
            ...
            self.profit = (
                self.num_actions * (self.open_price - self.close_price)
                - self.slippage
                - self.commission
            )
...
class TradeList:
    ...
    def get_current_profit(self, current_price, trade_type="all"):
        open_price = (
            lambda trade, p: trade.num_actions * (p - trade.open_price)
            - trade.commission
            - trade.slippage
        )
        ...
        if trade_type == "all":
            open_trade_prices = [
                open_price(v, current_price) for k, v in self.trades_opened.items()
            ]
            close_trade_prices = [v.profit for k, v in self.trades_closed.items()]
        else:
            open_trade_prices = [
                open_price(v, current_price)
                for k, v in self.trades_opened.items()
                if v.type == trade_type
            ]
            close_trade_prices = [
                v.profit for k, v in self.trades_closed.items() if v.type == trade_type
            ]

        return sum(open_trade_prices) + sum(close_trade_prices)

    def get_current_total_open(self, trade_type="all"):
        if trade_type == "all":
            open_trade_prices = [v.init_capital for k, v in self.trades_opened.items()]
            close_trade_prices = [v.init_capital for k, v in self.trades_closed.items()]
        else:
            open_trade_prices = [
                v.init_capital
                for k, v in self.trades_opened.items()
    ...
    def get_current_only_open(self, trade_type="all"):
        if trade_type == "all":
            open_trade_prices = [v.init_capital for k, v in self.trades_opened.items()]
        else:
            open_trade_prices = [
    ...
    def get_close_profit(self, trade_type="all"):
        if trade_type == "all":
            close_trade_prices = [v.profit for k, v in self.trades_closed.items()]
        else:
            close_trade_prices = [
    ...
    def get_close_trades_total_open(self, trade_type="all"):
        if trade_type == "all":
            close_trade_prices = [v.init_capital for k, v in self.trades_closed.items()]
        else:
            close_trade_prices = [
    ...
    def get_open_trades_total_open(self, trade_type="all"):
        if len(self.trades_opened) == 0:
            return 0

        if trade_type == "all":
            open_trade_prices = [v.open_price for k, v in self.trades_opened.items()]
        else:
            open_trade_prices = [
                v.open_price
                for k, v in self.trades_opened.items()
